...
class TradingStrategy:
    ...
    def monitor_and_execute_exits(self):
        ...
        try:
            ...
            # Registrar ordem criada no estado para monitoramento futuro
            if not hasattr(self.state.state, 'pending_exit_orders'):
                self.state.state['pending_exit_orders'] = []
            
            self.state.state['pending_exit_orders'].append({
    ...
    def check_executed_orders(self):
        """Verifica se alguma ordem de saída foi executada e registra no Google Sheets"""
        try:
            pending_orders = self.state.state.get('pending_exit_orders', [])
            
            if not pending_orders:
                return
            
            log(f"🔍 Verificando {len(pending_orders)} ordens pendentes...", "DEBUG")
            ...
            for order_info in pending_orders:
                try:
                    ...
                    # Buscar ordem
                    order = self.exchange.hyperliquid.fetch_order(order_id, symbol)
                    
                    # Verificar se foi executada
                    if order['status'] == 'closed' or order['status'] == 'filled':
                        # Ordem executada! Registrar venda
                        ...
                        log(f"✅ Ordem executada: {order_type.upper()} - {coin} @ ${price:.4f}", "INFO")
                        
                        # Registrar venda no estado e Google Sheets
                        self.state.record_sell(price, amount, coin, operation)
                        
                        # Notificar Discord
                        emoji = "🔴" if order_type == "stop_loss" else "🟢"
                        title = f"{emoji} {order_type.replace('_', ' ').upper()} EXECUTADO - {coin}"
                        
                        discord.send(
                            title,
                            f"**Preço de saída:** ${price:.4f}\n"
                            f"**Quantidade:** {amount:.4f} {coin}\n"
                            f"**Operação:** {operation}",
                            0xff0000 if order_type == "stop_loss" else 0x00ff00
                        )
                        
                        # Limpar position_entries se fechou completamente
                        self.state.state["position_entries"] = []
                        self.state.save_state()
                        
                    elif order['status'] == 'canceled':
                        # Ordem cancelada, remover da lista
                        log(f"⚠️ Ordem cancelada: {order_type.upper()} - {order_info['coin']}", "DEBUG")
                    else:
                        # Ordem ainda ativa
                        remaining_orders.append(order_info)
                        
                except Exception as e:
                    log(f"⚠️ Erro verificando ordem {order_info.get('order_id')}: {e}", "DEBUG")
                    # Manter na lista para tentar novamente
                    remaining_orders.append(order_info)
            
            # Atualizar lista de ordens pendentes
            self.state.state['pending_exit_orders'] = remaining_orders
            self.state.save_state()
            
        except Exception as e:
            log(f"❌ Erro verificando ordens executadas: {e}", "ERROR")
